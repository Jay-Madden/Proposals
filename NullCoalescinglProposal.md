...
For this addition to the language, no grammar changes are required. We are merely adding supported types to an existing operator. All current rules associated with the operator will still apply with the exception of the relaxed type rules, which will be updated.
